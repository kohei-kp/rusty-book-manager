...
impl CheckoutRepository for CheckoutRepositoryImpl {
    ...
    async fn create(&self, event: CreateCheckout) -> AppResult<()> {
        ...
        // 貸出処理を行う、すなわち checkouts テーブルにレコードを追加する。
    ...
    async fn update_returned(&self, event: UpdateReturned) -> AppResult<()> {
        ...
        // データベース上の返却操作として、
        // checkoutsテーブルになる該当貸出IDのレコードを、
        // returned_at を追加して returned_checkouts テーブルにINSERTする。
        let res = sqlx::query!(
            r#"
        ...
        // 上記処理が成功したら、checkouts テーブルから該当レコードを削除する。
        let res = sqlx::query!(
            r#"
    ...
    async fn find_unreturned_all(&self) -> AppResult<Vec<Checkout>> {
        // checkouts テーブルになるレコードを全件抽出する。
        // bookts テーブルと INNER JOIN し、蔵書の情報も一緒に抽出する。
...
impl CheckoutRepositoryImpl {
    ...
    // トランザクション分離レベルをSERIALIZABLEに設定するために内部的に使うメソッド
    ...
    // find_history_by_book_idで未返却の貸出し情報を取得するためのメソッド
